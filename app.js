...
1. チャットのメッセージ欄に、修正後のJSONデータや \`\`\`json などのブロックを直接書き出すことは「絶対に」禁止します。
2. 修正したJSONデータは、以下のPythonコード（Code Execution機能）をあなたが実行して、必ず「modified_project.txt」というテキストファイルとして保存してください。
   (※必ず以下のPythonコードブロックをあなたの内部のCode Executionで実行（Run）して、ファイルを生成してください)

\`\`\`python
# 修正したJSONデータを「modified_project.txt」というファイル名で保存する
json_data = """[ここに修正したJSONデータを入れる]"""
with open("modified_project.txt", "w", encoding="utf-8") as f:
    f.write(json_data)
\`\`\`

3. 【超重要】チャット欄にダウンロード用のマークダウンリンク（例：[modified_project.txt] や https://... など）は絶対に自分で書かないでください。Pythonを実行するだけで、Geminiのチャットシステムが自動的にファイルのダウンロードカードを表示してくれます。
4. ファイルに書き込むJSONは、出力サイズを節約するために、以下の省略ルールを必ず守ってください：
   - 変更（修正・改造）を加えたスプライトのみ、プログラム（blocks）を正しく記述してください。
   - 変更のない（元のプログラムのままでよい）スプライトは、プログラム（blocks）の中身を空オブジェクト \`{}\` に省略してください。
5. 「はい、直しました」などのあいさつや説明、余計なテキストは、ファイル内にもチャット内にも一切書かないでください。
`;
    } else {
      formatRuleText = `【AIの先生へのルール（必ずまもってください）】
